<?php
/**
 * @version   $Id: RokMiniEvents.php 8827 2013-03-27 14:58:21Z steph $
...
class RokMiniEvents
{
    ...
    protected function setTimezone(JRegistry $params, &$events)
    {
        ...
        $timezone = self::_get_timezone_offset($server_timezone);

        $localtime = $params->get('localtime', 'local');
        if ($localtime == 'local' && $user->id != 0) {
            //timezone is set to local and user is logged in
            $timezone = self::_get_timezone_offset($local_timezone);
        } else if ($localtime == 'forced') {
            //timezone is set to forced
            $timezone = self::_get_timezone_offset($forced_timezone);
        }
        if (!empty($events)) {
            foreach ($events as $event) {
                $event->setOffset($timezone);
            }
        }
        return $events;
    }
