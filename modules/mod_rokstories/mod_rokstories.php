<?php
/**
 * @version   $Id: mod_rokstories.php 8646 2013-03-21 02:25:00Z steph $
...
if (modRokStoriesHelper::checkRequest()) require(JModuleHelper::getLayoutPath('mod_rokstories', 'ajax')); else {
	...
	if ($iebrowser && $params->get("load_css", "1") == "1") {
		...
		if (file_exists($check . ".css")) $doc->addStyleSheet($style . ".css"); elseif (file_exists($check . ".php")) $doc->addStyleSheet($style . ".php");
	}
	...
	if ($conf->get('caching') && $params->get('module_cache')) {
		...
		$helper = new modRokStoriesHelper();
